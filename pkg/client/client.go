...
func (c *TransferClient) Connect(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("解析IPv4地址失败: %v", err)
	}
	log.Printf("解析到服务器IPv4地址: %s", ipAddr.String())
	...
	// 尝试建立 QUIC 连接
	log.Printf("正在建立QUIC连接到 %s...", raddr.String())
	...
	log.Printf("成功连接到QUIC服务器 %s", raddr.String())
	return nil
}
...
func (c *TransferClient) SendInitRequest() error {
	...
	// 解析响应
	respLen, cmd, dataLen, result, body := c.parseMessageByAES(responseBuffer, n, initAESKey)
...
func (c *TransferClient) SendTransferRequest(content string) ([]byte, error) {
	...
	for {
		...
		respLen, cmd, dataLen, result, body := c.parseMessageByAES(responseBuffer, currentSize, initAESKey)
		log.Printf("解析响应 - 长度: %d, 命令: %d, 数据长度: %d, 结果: %d", respLen, cmd, dataLen, result)
...
func (c *TransferClient) SendInitRequestNoAES() error {
	...
	// 发送请求
	written, err := c.stream.Write(initBytes)
	if err != nil {
		return fmt.Errorf("发送初始化请求失败: %v", err)
	}
	...
	for retry := 0; retry < maxRetries; retry++ {
		...
		// 解析响应
		respLen, cmd, dataLen, result, body = parseMessage(responseBuffer[:n], n)
		log.Printf("解析响应 - 长度: %d, 命令: %d, 数据长度: %d, 结果: %d", respLen, cmd, dataLen, result)
...
func (c *TransferClient) SendTransferRequestNoAES(content string) ([]byte, error) {
	...
	for retry := 0; retry < maxRetries; retry++ {
		...
		// 发送请求
		written, err := c.stream.Write(requestInfo)
		if err != nil {
