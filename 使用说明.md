...
```
require github.com/laotiannai/quic_gwclient v1.0.0  // 标准版，详细日志
```

```
require github.com/laotiannai/quic_gwclient v1.0.0-lite  // 轻量版，最小日志
```
...
```bash
# 引用特定分支
go get github.com/laotiannai/quic_gwclient@v1.0.0_release  # 标准版分支
go get github.com/laotiannai/quic_gwclient@v1.0.0_lite  # 轻量版分支
...
- **标准版**: 详细的日志输出，适合调试和开发环境
- **轻量版**: 最小化日志输出，只显示警告和错误，适合生产环境

### 2. 日志级别控制

除了选择不同版本，还可以通过以下方式精细控制日志级别：

#### 环境变量控制

```bash
# 设置日志级别
# 可选值: none, error, warning, info, debug
...
| 日志级别 | 说明 | 适用场景 |
|---------|------|---------|
| LogLevelNone | 禁用所有日志 | 对性能要求极高的生产环境 |
| LogLevelError | 只显示错误信息 | 一般生产环境 |
| LogLevelWarning | 显示警告和错误 | 轻量版默认级别，普通生产环境 |
| LogLevelInfo | 显示一般信息、警告和错误 | 一般测试环境 |
| LogLevelDebug | 显示所有日志，包括调试信息 | 标准版默认级别，开发调试环境 |
...
```
2023/06/15 14:32:45.123456 [DEBUG] 创建新的QUIC客户端 - 服务器地址: example.com:8002, 服务器ID: 8903, 服务器名称: test_server
2023/06/15 14:32:45.234567 [INFO] 正在连接QUIC服务器 example.com:8002...
2023/06/15 14:32:45.345678 [DEBUG] 连接配置信息 - ServerID: 8903, ServerName: test_server, SessionID: abac17fd-e8e0-4600-b822-09f5755148d7
2023/06/15 14:32:45.456789 [INFO] QUIC连接建立成功，远程地址: 192.168.1.100:8002
2023/06/15 14:32:45.567890 [DEBUG] QUIC流打开成功
2023/06/15 14:32:45.678901 [INFO] 初始化请求已发送
2023/06/15 14:32:45.789012 [INFO] 初始化请求成功
```

#### 轻量版（LogLevelWarning）

```
2023/06/15 14:32:45.234567 [WARN] QUIC Gateway Client 初始化 - 轻量版
2023/06/15 14:32:45.456789 [ERROR] 传输请求失败 (尝试 1/3): context deadline exceeded
``` 