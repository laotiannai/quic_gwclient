...
2. **关于文件保存路径**：
   - 当 `SaveToFile` 为 `true` 时，文件保存路径由 `SaveDir` 和 `FileNamePrefix` 以及内容的MD5值组成
   - 最终格式为: `SaveDir/FileNamePrefix_MD5.bin`
   - 例如: `./downloads/document_8f7df918c0b72a3b5def87.bin`
   - 当 `SaveToFile` 为 `false` 时，返回格式为: `memory:SaveDir/FileNamePrefix_MD5.bin`
...
filePath, err := client.DownloadFile(httpRequest, "./downloads", "myfile", true)
if err != nil {
    log.Fatalf("下载失败: %v", err)
}
